...
import AiAnalysisModule from "../ai-analysis/ai-analysis";
...
type CompanyRecord=[string,string,number|null,string,string,string,string,string,string];
...
function IndustryDonut({composition:compositions,relation,parent,onIndustrySelect,selectedIndustry,companyPanel}:{composition:{count:IndustryComposition|null;amount:IndustryComposition|null};metric:"count"|"amount";relation:string;parent:string;onIndustrySelect?:(industry:{level:number;code:string;name:string})=>void;selectedIndustry?:CompanyIndustry;companyPanel?:any}){
  const countComposition=compositions.count,amountComposition=compositions.amount;
  const composition=countComposition||amountComposition;
  ...
  const levelName=["","一级","二级","三级"][composition.level],title=composition.title||`${levelName}行业占比`;
  const countItems=new Map((countComposition?.items||[]).map(item=>[item.code,item]));
  const amountItems=new Map((amountComposition?.items||[]).map(item=>[item.code,item]));
  const legendItems=[...new Map([...(countComposition?.items||[]),...(amountComposition?.items||[])].map(item=>[item.code,item])).values()];
  const chart=(current:IndustryComposition|null,metric:"count"|"amount")=>{
    if(!current?.items.length)return <div className="industryDonutPlaceholder">暂无{metric==="amount"?"金额":"关系数量"}数据</div>;
    const edges=current.items.reduce<number[]>((values,item)=>[...values,values[values.length-1]+item.share*100],[0]);
    const gradient=current.items.map((item,index)=>`${item.color} ${edges[index]}% ${edges[index+1]}%`).join(",");
    return <div className="industryDonutFigure"><div className="industryDonut" style={{background:`conic-gradient(${gradient})`}} role="img" aria-label={`${title}${metric==="amount"?"金额":"关系数量"}环形图，共${current.items.length}类`}><span><strong>{fmt(current.total,metric==="amount"?1:0)}</strong><small>{metric==="amount"?"万元":"条"}</small></span></div><b>{metric==="amount"?"金额占比":"关系数量占比"}</b></div>;
  };
  if(!legendItems.length)return <section className="industryDonutCard empty"><div><h4>{title}</h4><p>当前关系在所选产业口径中没有可汇总记录。</p></div></section>;
  return <section className="industryDonutCard">
    <div className="industryDonutHead"><h4>{title}</h4><p>{composition.context||parent} · {relation==="专利"?"按行业关联次数":"双向关系合计；两图使用同一行业颜色"}</p></div>
    <div className="industryDonutBody comparison">
      <div className="industryDonutCharts">{chart(countComposition,"count")}{relation!=="专利"&&chart(amountComposition,"amount")}</div>
      <div className="industryDonutLegend shared">{legendItems.map(item=>{const count=countItems.get(item.code),amount=amountItems.get(item.code);return <button type="button" disabled={!onIndustrySelect||item.code==="UNCLASSIFIED"} className={selectedIndustry?.code===item.code?"active":""} key={item.code} onClick={()=>onIndustrySelect?.({level:composition.level,code:item.code,name:item.name})}><i style={{background:item.color}}/><span><strong>{item.code==="UNCLASSIFIED"?"未分类":`${item.code} · ${item.name}`}</strong><small>{count?`${fmt(count.value)}条 · ${fmt(count.share*100,1)}%`:"关系数 0"}{relation!=="专利"&&`｜${amount?`${fmt(amount.value,1)}万元 · ${fmt(amount.share*100,1)}%`:"金额 0"}`}</small></span></button>})}</div>
    </div>
    {companyPanel}
  </section>;
}
...
export default function Home(){
  const [platformFeature,setPlatformFeature]=useState<"区域联系网络"|"联系象限"|"用地分析"|"公共服务设施"|"智能分析">("区域联系网络");
  ...
  const [analysisLevel,setAnalysisLevel]=useState<"城市"|"区县"|"镇街">("区县");
  ...
  const industryCompositions=useMemo<{count:IndustryComposition|null;amount:IndustryComposition|null}>(()=>{
    if(!data||!selected)return{count:null,amount:null};
    const compositionRecords:Array<Rec|TownRec>=analysisLevel==="镇街"?(allTownData?.records||[]):data.records;
    const build=(compositionMetric:"count"|"amount"):IndustryComposition=>{
    const matchesSelected=(record:Rec|TownRec)=>{
      if(analysisLevel==="城市"){
        const r=record as Rec;return (r[4]===selected.oc&&r[6]===selected.dc)||(r[4]===selected.dc&&r[6]===selected.oc);
      }
      if(analysisLevel==="镇街"){
    ...
    const recordValue=(record:Rec|TownRec)=>analysisLevel==="镇街"?(compositionMetric==="count"?(record as TownRec)[10]:(record as TownRec)[11]):(compositionMetric==="count"?(record as Rec)[8]:(record as Rec)[9]);
  ...
  const mainCenter=(flow:Flow,side:"o"|"d")=>{
    ...
    if(analysisLevel==="城市")return geo?.cityCenters[side==="o"?flow.oc:flow.dc];
  ...
  const mainLabelCandidates=[...new Map(shown.flatMap((flow,index)=>[
    {key:analysisLevel==="城市"?flow.oc:analysisLevel==="区县"?`${flow.oc}|${flow.o}`:`${flow.oc}|${flow.o}|~${normalizeTown(flow.ot||"")}`,name:analysisLevel==="城市"?flow.oc:analysisLevel==="区县"?flow.o:flow.ot||flow.o,point:mainCenter(flow,"o"),priority:shown.length-index,selected:selected?.key===flow.key},
    {key:analysisLevel==="城市"?flow.dc:analysisLevel==="区县"?`${flow.dc}|${flow.d}`:`${flow.dc}|${flow.d}|~${normalizeTown(flow.dt||"")}`,name:analysisLevel==="城市"?flow.dc:analysisLevel==="区县"?flow.d:flow.dt||flow.d,point:mainCenter(flow,"d"),priority:shown.length-index,selected:selected?.key===flow.key}
  ]).map(item=>[item.key,item] as const)).values()].filter(item=>Boolean(item.point)) as MapLabelCandidate[];
  const townLabelCandidates=[...new Map(displayedTownFlows.flatMap((flow,index)=>[
  ...
  const townMapSubtitle=`${activeIndustryPath} · ${townMetric==="count"?"关系数量":"金额（万元人民币）"} · ${townLimit===0?`全部 ${townFlows.length} 条`:`前 ${Math.min(townLimit,townFlows.length)} / ${townFlows.length} 条`}镇街联系`;
  const performanceSource=relation==="专利"?flows.flatMap(flow=>[flow,{...flow,oc:flow.dc,o:flow.d,ot:flow.dt,dc:flow.oc,d:flow.o,dt:flow.ot}]):flows;
  const performanceRows=buildRegionPerformance(performanceSource,flow=>analysisLevel==="城市"?flow.oc:analysisLevel==="区县"?flow.o:flow.ot||flow.o,flow=>metric==="count"?flow.count:flow.amount);
  const activeProfile=performanceRows.some(row=>row.name===profileRegion)?profileRegion:(performanceRows[0]?.name||"");
  const loadCompanies=async(next:{level:number;code:string;name:string})=>{
    if(!selected||analysisLevel!=="区县"||relation==="专利")return;
    setCompanyIndustry(next);setCompanyLoading(true);setCompanyRecords([]);
    try{
      const index=await fetch("/data/enterprise-companies/index.json").then(response=>response.json()) as {pairs:Record<string,string>};
      const directions=[
        {key:[relation,selected.oc,selected.o,selected.dc,selected.d].join("|"),label:`${selected.o} → ${selected.d}`},
        {key:[relation,selected.dc,selected.d,selected.oc,selected.o].join("|"),label:`${selected.d} → ${selected.o}`}
      ];
      const payloads=await Promise.all(directions.map(async direction=>{const file=index.pairs[direction.key];if(!file)return{direction,records:[] as CompanyRecord[]};const payload=await fetch(`/data/enterprise-companies/${file}`).then(response=>response.json()) as {records:CompanyRecord[]};return{direction,records:payload.records}}));
      const codeIndex=next.level===1?3:next.level===2?5:7;
      const rows=payloads.flatMap(({direction,records})=>records.filter(record=>record[codeIndex]===next.code).map(record=>({o:record[0],d:record[1],amount:record[2],direction:direction.label})));
      rows.sort((a,b)=>(b.amount??-Infinity)-(a.amount??-Infinity));setCompanyRecords(rows);
  ...
  const headerTitle=platformFeature==="智能分析"?"厦漳泉都市圈智能分析":platformFeature==="用地分析"?"厦漳泉用地结构分析":platformFeature==="公共服务设施"?"厦漳泉公共服务设施图谱":platformFeature==="联系象限"?"厦漳泉区县联系四象限":module==="企业关系"?"厦漳泉企业关系流动图谱":module==="人口流动"?"厦漳泉人口流动图谱":"厦漳泉交通可达性";
  const headerSubtitle=platformFeature==="智能分析"?"调用 DeepSeek / 通义千问，基于平台五类数据生成都市圈发展分析报告":platformFeature==="用地分析"?"三市28区县开发强度、功能结构、混合度与区位熵比较":platformFeature==="公共服务设施"?"基于POI中类的区县设施数量、空间分布与结构比较":platformFeature==="联系象限"?"人口流动联系与企业联系相对引力预期的协同类型识别":module==="企业关系"?"城市、区县与镇街三级投资、分支与专利协作网络 · 已排除区域内部关系":module==="人口流动"?"城市、区县与镇街三级人口流动方向、双向联系与净流动分析":"城市、区县与乡镇街三级政府驻地驾车时间、距离、过路费分析";
  const headerStatus=platformFeature==="智能分析"?"AI 报告 · 数据问答":platformFeature==="用地分析"?"单期横截面 · 28 个区县":platformFeature==="公共服务设施"?"230 个POI中类 · 28 个区县":platformFeature==="联系象限"?"四类象限 · 378 个区县对":module==="企业关系"?"3 类关系 · 28 个区县":module==="人口流动"?"人口流动 · 28 个区县":"驾车 OD · 区县与镇街";
  const headerNote=platformFeature==="智能分析"?"结论先行 · 数据落到产业/功能/用地门类":platformFeature==="用地分析"?"面积统一采用公顷":platformFeature==="公共服务设施"?"现行区县名称汇总":platformFeature==="联系象限"?"优化引力模型 · 无向区县对":module==="企业关系"?`汇率基准 ${data.meta.baseline}`:module==="人口流动"?"行政区按源文件名称汇总":"区县756条 · 镇街108,570条有向OD";
  ...
  const platformSwitch=<nav className="platformSwitch" aria-label="平台功能"><button className={platformFeature==="区域联系网络"?"active":""} onClick={()=>setPlatformFeature("区域联系网络")}><span>区域联系网络</span><small>企业 · 人口流动 · 交通</small></button><button className={platformFeature==="用地分析"?"active":""} onClick={()=>setPlatformFeature("用地分析")}><span>用地分析</span><small>结构 · 强度 · 区位熵</small></button><button className={platformFeature==="公共服务设施"?"active":""} onClick={()=>setPlatformFeature("公共服务设施")}><span>公共服务设施</span><small>POI中类 · 区县比较</small></button><button className={platformFeature==="联系象限"?"active":""} onClick={()=>setPlatformFeature("联系象限")}><span>联系象限</span><small>引力模型 · 协同类型</small></button><button className={platformFeature==="智能分析"?"active":""} onClick={()=>setPlatformFeature("智能分析")}><span>智能分析</span><small>AI 报告 · 数据问答</small></button></nav>;
  ...
  if(platformFeature==="智能分析")return <main>{header}{platformSwitch}<AiAnalysisModule/><footer><span>数据源：企业关系 · 人口流动 · 交通可达性 · 用地结构 · 公共服务设施 · 引力模型四象限（经 build_ai_summary.mjs 聚合成 AI 摘要）</span><span>说明：AI 报告基于平台数据生成，数字口径见各模块；关键数据由 keyPairs 区县对画像表提供，供模型直接引用</span></footer></main>;
  ...
  if(module==="交通可达性")return <main>{header}{platformSwitch}<TransportAccessibilityModule toolbar={moduleSwitch}/><footer><span>数据源：厦漳泉区县及乡镇街政府驻地驾车 OD · 福建省 GeoPackage 行政边界</span><span>距离口径：有向地图保留实际方向；无向统计取两方向驾车里程算术平均值</span></footer></main>;
  if(module==="人口流动")return <main>{header}{platformSwitch}<PopulationModule toolbar={moduleSwitch}/><footer><span>数据源：厦漳泉乡镇级人口流动_汇总版.xlsx · 福建省 GeoPackage 行政边界</span><span>人口流动口径：不使用行政区代码，O/D 地市、区县和镇街均按源文件名称汇总</span></footer></main>;
  return <main>{header}{platformSwitch}
    <div className="moduleTopRow">{moduleSwitch}<section className="controls">
    ...
    {townOpen&&selected&&analysisLevel==="区县"&&<div className="townOverlay" role="dialog" aria-modal="true" aria-label={`${selected.o}与${selected.d}镇街联系`} onMouseDown={()=>setTownOpen(false)}>
      <section className="townPanel" onMouseDown={e=>e.stopPropagation()}>
        ...
        {townLoading?<div className="townState townLoadingState">正在载入镇街关系与边界地图…</div>:townError?<div className="townState error">{townError}</div>:<>
          ...
          {townFlows.length===0?<div className="townState">当前筛选下没有两端镇街均完整的关系记录。</div>:<div className="townTable"><div className="townTableHead"><span>序号</span><span>O端镇街</span><span>方向</span><span>D端镇街</span><span>关系数量</span><span>{relation==="专利"?"专利名 / 企业对":"金额（万元）"}</span></div>{displayedTownFlows.map((x,index)=><article key={x.key}><b>{String(index+1).padStart(2,"0")}</b><div><strong>{mapDisplayName(x.ot)}</strong><small>{x.oc} · {x.o}</small></div><i>{relation==="专利"?"—":"→"}</i><div><strong>{mapDisplayName(x.dt)}</strong><small>{x.dc} · {x.d}</small></div><em>{fmt(x.count)}</em><div className="townValue">{relation==="专利"?<><strong>{fmt(x.patents)} / {fmt(x.pairs)}</strong><small>唯一专利名 / 企业对</small></>:<><strong>{fmt(x.amount,2)}</strong><small>{fmt(x.amountCount)} 条有金额</small></>}</div></article>)}</div>}
