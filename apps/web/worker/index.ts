...
- **可用定位词汇表（每个定位必须引用其对应数据路径）**：
  ...
  - "产业节点" **必须指明具体产业链**（纺织鞋服与消费品/建材家居/石化医药与新材料/先进装备制造/电子信息与数字/能源资源与生态公用/建筑与房地产/商贸物流/金融商务与租赁/科技教育医疗/生活文旅与社会服务/农业食品）→ 引用该产业链的条数/金额/跨市占比（industry.byChain、chainByScope）+ 工业及仓储占比、产业开发密度；
...
- **定位名称必须与引用数据精确对应**：定位里出现的每个词都要能在"数据："里有直接数字（如"纺织鞋服产业节点"必须有"纺织鞋服与消费品产业链 X 条/金额 X 万元、跨市占比 X%"；"科教医疗中心"必须有教育/医疗类 POI 或科技教育医疗产业链数据）；找不到对应数据的词不写；
