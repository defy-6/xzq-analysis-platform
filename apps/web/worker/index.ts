...
const TOPIC_SECTIONS: Record<string, { label: string; paths: string[][] }> = {
  ...
  insights: {
    ...
    paths: [
      ...
      ["industry", "byChainCounty"], ["services", "byFunctionalCounty"],
    ],
  },
  chat: {
    label: "数据问答",
...
- **产业分析必须条数×金额并重，禁止只看关系条数**：判断某产业/产业链/区县在某方向的产业地位时，须同时给出关系条数与金额，并参考单条平均金额（avgAmount，单位万元/条，= 金额 ÷ 投资/分支条数）。警惕"大额少条"形态：条数少但金额占比高（单条平均金额显著高于全区县/全产业链均值）时，说明该产业由少数龙头大项目驱动（如大型石化、装备制造、能源项目），不能用条数衡量其地位，也不能只报条数或只报金额，必须两者结合并明确点出"大额项目驱动"特征；反之"条多额小"（单条金额远低于均值）说明产业联系广泛分散，多为中小企业网络。
...
- 对每个产业/产业链，**条数与金额必须同时呈现**（含 avgAmount 单条平均金额），并判断其形态：是大额少条（条数占比明显低于金额占比、单条金额高，多为龙头大项目）还是广泛分散（条数多、单条金额低，多为中小企业网络）；涉及制造业等资本密集型行业尤其要检查是否存在"投资额巨大但关系数少"的情况；
...
- **可用定位词汇表（每个定位必须引用其对应数据路径）**：
  ...
  - "产业节点" **必须指明具体产业链**（纺织鞋服与消费品/建材家居/石化医药与新材料/先进装备制造/电子信息与数字/能源资源与生态公用/建筑与房地产/商贸物流/金融商务与租赁/科技教育医疗/生活文旅与社会服务/农业食品）→ 优先引用 **industry.byChainCounty**（该区县在该产业链的条数/金额/单条平均金额 avgAmount/跨市占比，如"晋江市·纺织鞋服与消费品 26 条、金额 1.2 亿元、单条平均 460 万元、跨市占比 38%"）；区县级缺失时用城市级 industry.byChain、chainByScope 佐证，再补工业及仓储占比、产业开发密度；**条数与金额必须并重：若该区县该产业链条数少而金额大（avgAmount 显著高于整体），定位表述为"X 龙头大项目承载地/大额投资节点"而非"产业网络密集区"，并同时给出条数与金额；**
  - "商贸物流节点" → 商贸物流产业链、商业及物流仓储用地占比、交通门户指数 + **商业消费/物流服务功能 POI（byFunctionalCounty，数量/每万人/28 区县排名/均值倍数）**；
  - "金融商务中心" → 金融商务与租赁产业链数据、商业占比 + **商业消费/产业就业功能 POI（byFunctionalCounty）**；
  - "科教医疗中心" → 科技教育医疗产业链、公共服务用地占比 + **教育文化/医疗健康功能 POI（byFunctionalCounty）**；
...
- **定位须论证相对位置**：每条定位的"数据："除绝对数值外，必须给出该维度在 28 区县中的相对位置（如"商业消费 POI 居 28 区县第 2、为区县均值 3.9 倍""教育文化每万人 POI 为均值 1.6 倍"），用相对位置说明**该定位为何在此区县成立**；若该区县在该维度上低于区县均值或排名靠后（rank 大于 14 或 multiple 小于 1），不得用该维度支撑定位；
...
【特色重点方向】（优先挖掘）
- **"大额少条"型产业**：某区县某产业链关系条数少、但金额/单条平均金额独高（avgAmount 显著高于整体，或金额占比远高于条数占比），如石化、能源、装备类龙头大项目——条数看不出、只有金额能揭示的独特产业地位；
- **区县在都市圈中的独特/垄断地位**：某区县在某个产业链或功能服务中居 28 区县首位或排名极高（rank 第 1-3、multiple 远超均值、金额/POI 数量占都市圈总量比例突出，如占某产业链投资额过半、某功能 POI 一家独大），形成该领域不可替代的节点——重点关注由 byChainCounty（区县×产业链，含条数/金额/单条均值/跨市占比）与 byFunctionalCounty（区县×功能 POI，含排名/均值倍数）揭示的"看似不起眼、实则独树一帜"的区县。
【数据证据要求】每条发现的"数据证据"必须给出该维度在 28 区县中的相对位置（排名/均值倍数/占比），不得只报绝对数值。
